...
def clusterize(batch, k=4, MAX_ITER=1000, epsilon=10**-3):
    start = time.time()
    ...
    def get_function(points, mu, sigma): # f_ik [n,k]
        div_tmp = coef*tf.rsqrt(tf.batch_matrix_determinant(sigma)) # ((2pi)^p*|S_k|)^-1/2  [k]
...
if __name__ == "__main__":
    clusterize(tf.random_uniform([100,100,1]).eval(session=tf.Session()))
